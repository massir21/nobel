...
if(isset($_GET['frompresupuesto'])){
    $nombre=$registros[0]['nombre']." ".$registros[0]['apellidos']." (".$registros[0]['telefono'].")";
    ?>
    <script type="text/javascript">
        console.log(window.opener);
        window.opener.location.href="/presupuestos/nuevo_presupuesto?idcliente=<?php echo $_GET['frompresupuesto'];?>&cliente=<?php echo $nombre;?>";
        window.close();
    </script>
    <?php
    return ;
...
?>
<?php
$tiposDocumentos=['Plan de tratamiento', 'Anamnesis', 'Consentimiento', 'Ficha antigua', 'Panoramica',
    'Informe médico', 'Informe medicacion', 'Consentimientos de reconstruccion',
    'Consentimiento limpieza', 'DNI', 'Datos bancarios', 'Nomina', 'Pasaporte de implantes',
    'Solicitud devolución','Garantías','Documentos varios','Ficha Ortodoncia','Fotografías'];
asort($tiposDocumentos);
/*
$tiposDocumentosPopup=['Panoramica', 'Informe médico', 'Informe medicacion', 'Consentimientos de reconstruccion',
    'Consentimiento limpieza', 'DNI', 'Datos bancarios', 'Nomina', 'Pasaporte de implantes','Garantías','Dcoumentos varios'];
asort($tiposDocumentosPopup);
*/
$tiposDocumentosPopup=$tiposDocumentos;
?>
<style>
    .dataTables_filter {
        text-align: right;
...
<div class="card mb-5">
    <div class="card-body">
        <div class="d-flex flex-center flex-row mb-5">
            <h1 class="fs-3 text-gray-800 fw-bold mb-1">
                ...
                <?php if ($this->session->userdata('id_perfil') != 6&&$this->session->userdata('id_perfil') != 1) { ?><a class="btn btn-sm btn-icon btn-warning text-hover-warning" href="<?php echo base_url(); ?>clientes/gestion/editar/<?php echo $id_cliente; ?>" data-bs-toggle="tooltip" title="Editar cliente"><i class="fa-regular fa-pen-to-square"></i></a><?php } ?>
            </h1>
        </div>
    ...
    <div class="card-footer px-5 py-0">
        <ul class="nav nav-stretch nav-line-tabs nav-line-tabs-2x border-transparent fs-6 fw-bold" role="tablist" id="ul_tab_nav">
            ...
            <li class="nav-item" role="presentation"><a class="nav-link text-active-primary py-5 me-6" aria-selected="false" tabindex="-1" role="tab" href="#tab2default" data-bs-toggle="tab">H.Clínica</a></li>
            ...
            <li class="nav-item" role="presentation"><a class="nav-link text-active-primary py-5 me-6" aria-selected="false" tabindex="-1" role="tab" href="#tab9default" data-bs-toggle="tab">Fichas</a>
            </li>
            ...
            <li class="nav-item" role="presentation"><a class="nav-link text-active-primary py-5 me-6" aria-selected="false" tabindex="-1" role="tab" href="#tab5default" data-bs-toggle="tab">N.Citas</a>
            </li>
            ...
            <li class="nav-item" role="presentation"><a class="nav-link text-active-primary py-5 me-6" aria-selected="false" tabindex="-1" role="tab" href="#tab7default" data-bs-toggle="tab">Saldo</a>
            </li>
    ...
    <div class="card-body">
        <div class="tab-content">
                                        ...
                                        <tr style="background: <?php echo $row['color_estado']; ?>;">
                                            ...
                                            <td style="text-align: center; <?= ($row['producto'] != "") ? 'background: #fad7e4;' : '' ?> <?= ($row['descuento_euros'] > 0 || $row['descuento_porcentaje'] > 0) ? 'background: #dda6fa;' : '' ?> <?= ($row['codigo_proveedor'] != "") ? 'background: #f9ca8e;' : '' ?>">
                                                ...
                                                <?php if ($row['id_presupuesto'] > 1) {
                                                    echo '<a href="javascript:void(0);" class="viewpresu" 
                                                        data-idpresupuesto="'.$row['id_presupuesto'].'"
                                                        data-numpresupuesto="'.$row['nro_presupuesto'].'"
                                                        title="Ver presupuesto">';
                                                    echo "Presupuesto " . $row['nro_presupuesto'];
                                                    echo " - ";
                                                    echo number_format($row['totalpresupuesto'],2,".","")."€";
                                                    echo '</a>';
                                                } ?>
                                                <?php
            ...
            <div class="tab-pane fade" id="tab2default">
                ...
                <?php // foreach con los evolutivos encontrados, ordenados de mas nuevo a mas viejo 
                ?>
                        ...
                        <div class="d-flex flex-column border-1 border-dashed card-rounded p-5 p-lg-10 mb-14 item_evolutivo">
                            <div class="d-flex justify-content-between border-2 border-bottom border-dark-subtle mb-3">
                                ...
                                <h5 class="text-start text-danger fw-bold fs-5 text-uppercase gs-0">
                                    <span><?= $nota['fecha_nota_formateada'] ?>
                                    <?php
                                    if($this->session->userdata('id_perfil')==0 &&
                                        ($nota['fecha_nota_formateada']!=date("d-m-Y H:i",strtotime($nota['fecha_creacion'])))){
                                        ?>
                                        <span style="font-size:0.6em;color:darkgray"><?= date("d-m-Y H:i",strtotime($nota['fecha_creacion']));?></span>
                                    <?php
                                    }
                                    ?>
                                        </span>
                                    <?php if ($this->session->userdata('id_perfil') == 0 || ($nota['id_usuario_creador'] == $this->session->userdata('id_usuario') && $nota['fecha_creacion'] > date("Y-m-d H:i:s", strtotime("-1 day")))) { ?>
                                        ...
                                        <button type="button" class="btn btn-sm btn-warning btn-icon ms-3 editevol" idevol="<?= $nota['id'] ?>" idd="<?= $nota['id_usuario_doctor'];?>"
                                            ...
                                            <tr>
                                                <td>
                                                    <a href="<?php echo base_url(); ?>carnets/detalle/gestion/<?php echo $row['id_carnet']; ?>"
                                                        ...
                                                        <a
                                                            onclick="VerCarnetsPagos(<?php echo $row['id_carnet'] ?>);"><b><?php echo $row['codigo'] ?></b></a>
                                                    </a>
                                                </td>
                                            ...
                                            if ($carnets_vendidos != 0) {
                                                ...
                                                foreach ($carnets_vendidos as $key => $row) { ?>
                                                    <tr>
                                                        ...
                                                        <td style="padding-right: 5px;"> |
                                                            <?php $suma_total_precio += $row['total_precio'];
                                                            echo number_format($row['total_precio'], 2, ',', '.'); ?>
                                                            €</td>
                                                    </tr>
                                        <?php }
                                            }
                                        ...
                                        <?php if (isset($servicios_realizados)) {
                                            if ($servicios_realizados != 0) {
                                                foreach ($servicios_realizados as $key => $row) { ?>
                                                    <tr>
                                                        <td style="padding-right: 5px;">
                                                            <?php echo $row['nombre_familia'] . " - " . $row['nombre_servicio']; ?></td>
                                                        <td style="padding-right: 5px;"> | <?php echo $row['veces']; ?> veces</td>
                                                    </tr>
                                        <?php }
                                            }
                    ...
                    <table class="table align-middle table-striped table-row-dashed fs-6 gy-5">
                        ...
                        <tbody class="text-gray-700 fw-semibold">
                            <tr>
                                ...
                                <td>
                                    <button class="btn btn-sm btn-warning btn-icon" type="button" data-bs-toggle="collapse" data-bs-target="#collapse_form_ficha_cliente" aria-expanded="false" aria-bs-controls="collapse_form_ficha_cliente">
                                        <i class="fa-regular <?php if($this->session->userdata('id_perfil')==6){?>  fa-eye <?php } else { ?> fa-pen-to-square <?php } ?>"></i>
                                    </button>
                                </td>
                            </tr>
                        </tbody>
                ...
                <div class="collapse" id="collapse_form_ficha_cliente">
                    <form id="form" action="<?php echo base_url(); ?>clientes/gestion/actualizar/<?php echo $registros[0]['id_cliente'] ?>" role="form" method="post" name="form" class="border p-3">
                        <div class="row mb-5 border-bottom">
                            ...
                            <?php
                            if($this->session->userdata('id_perfil')==6){
                                // CHAINS 20240219 - El doctor no puede ver email ni telefono ni direccion
                            }
                            else{
                            ?>
                            <div class="col-md-4 mb-5">
                            ...
                            <div class="col-md-3 mb-5">
                                ...
                                <select name="sexo" id="sexo" class="form-select form-select-solid">
                                    <option value="Femenino" <?= ($registros[0]['sexo'] == "Femenino") ? 'selected' : '' ?>>
                                        Femenino</option>
                                    <option value="Masculino" <?= ($registros[0]['sexo'] == "Masculino") ? 'selected' : '' ?>>
                                        Masculino</option>
                                </select>
                            </div>
                        ...
                        <div class="row">
                            <div class="col-md-12 d-flex justify-content-between">
                                ...
                                <?php
                                if($this->session->userdata('id_perfil')==6){
                                 // CHAINS 20240219 - El doctor no puede modificar el cliente
                                }
                                else{
                                ?>
                                ...
                                <div class="row mb-5 border-bottom">
                                    ...
                                    <div class="col-md-4">
                                        ...
                                        <div class="form-check form-check-custom form-check-solid mb-3">
                                            ...
                                            <label class="form-check-label text-uppercase" for="pasadas2">si: Cáncer, hepatitis,
                                                ...</label>
                                        </div>
                                    </div>
                                ...
                                <div class="row mb-5 border-bottom">
                                    ...
                                    <div class="col-md-4">
                                        ...
                                        <div class="form-check form-check-custom form-check-solid mb-3">
                                            ...
                                            <label class="form-check-label text-uppercase" for="actuales2">si: Cáncer, VIH,
                                                hepatitis, autoinmunes, diabetes, tensión, ...</label>
                                        </div>
                                    </div>
                                ...
                                <div class="row mb-5 border-bottom">
                                    ...
                                    <div class="col-md-4">
                                        ...
                                        <div class="form-check form-check-custom form-check-solid mb-3">
                                            ...
                                            <label class="form-check-label text-uppercase" for="medicamentos2">si: Aspirina,
                                                diuréticos, corticoides, antiinflamatorios, antibióticos, antidepresivos,
                                                ansiolíticos, somníferos,...</label>
                                        </div>
                                    </div>
                                ...
                                <div class="row mb-5 border-bottom">
                                    ...
                                    <div class="col-md-4">
                                        ...
                                        <div class="form-check form-check-custom form-check-solid mb-3">
                                            ...
                                            <label class="form-check-label text-uppercase" for="suplementos2">si: Vitaminas,
                                                diuréticos, homeopatía,...</label>
                                        </div>
                                    </div>
                                ...
                                <div class="row mb-5 border-bottom">
                                    ...
                                    <div class="col-md-4">
                                        ...
                                        <div class="form-check form-check-custom form-check-solid mb-3">
                                            ...
                                            <label class="form-check-label text-uppercase" for="intervenciones2">si: cirugía
                                                general, cirugía estética o plástica de reconstrucción, fracturas,
                                                esguinces,...</label>
                                        </div>
                                    </div>
                                ...
                                <div class="row mb-5 border-bottom">
                                    <div class="col-md-4">
                                        ...
                                        <div class="firma" style="position: relative; width: 300px; height: 150px; -moz-user-select: none; -webkit-user-select: none; -ms-user-select: none; user-select: none;">
                                            ...
                                            <input type="hidden" name="firma_img" id="firma_img" value="lsakdhlskdh" />
                                        </div>
                                        <a href="#" id="clear" onclick="clearSignature();return false;" class="btn btn-sm btn-danger">Borrar Firma</a>
                                    </div>
                                </div>
                    ...
                    <form id="form" action="<?php echo base_url(); ?>clientes/consentimiento/<?php echo $registros[0]['id_cliente'] ?>" role="form" method="post" name="form2" class="border p-3" onsubmit="return EsOk2();">
                        <div class="row mb-5 border-bottom">
                            <?php if (!isset($aleatorio)) { ?>
                                ...
                                <!-- Fin -->
                                <!-- 08/11/23 -->
                                <div class="col-md-4" id="descriptionTrata" style="display: none;">
                                    <label id="titleTratamiento" class="form-label">Tratamiento</label>
                                    <input type="text" name="tratamiento" id="tratamiento" class="form-control" placeholder="Describa el tratamiento" />
                                </div>
                                <div class="col-md-12" id="descriptionRiesgos" style="display: none;">
                                    <label id="titleRiesgos" class="form-label">Riesgos y complicaciones</label>
                                    <textarea id="riesgos" name="riesgos" rows="4" cols="50" class="form-control" placeholder="Describa los riesgos y complicaciones"></textarea>
                                </div>
                                <div class="col-md-4" id="descriptionProc" style="display: none;">
                                    <label id="titleProc" class="form-label">Procedimiento clínico</label>
                                    <input type="text" name="procedimiento" id="procedimiento" class="form-control" placeholder="Describa el procedimiento" />
                                </div>
                                <div class="col-md-8" id="descriptionProblema" style="display: none;">
                                    <label id="titleProbc" class="form-label">El problema que presenta el/la paciente consiste en</label>
                                    <input type="text" name="problema" id="problema" class="form-control" placeholder="Describa el problema que presenta el paciente" />
                                </div>
                                <div class="col-md-12" id="descriptionProcTerapeuticos" style="display: none;">
                                    <label id="titleTera" class="form-label">Procedimientos terapéuticos</label>
                                    <textarea id="procTerapeuticos" name="procTerapeuticos" rows="4" cols="50" class="form-control" placeholder="Describa los procedimientos terapéuticos alternativos propuestos al paciente"></textarea>
                                </div>
                                <div class="col-md-4" id="descriptionEn" style="display: none;">
                                    <label id="titleEn" class="form-label">En</label>
                                    <input type="text" name="en" id="en" class="form-control" placeholder="Lugar de tratamiento" />
                                </div>
                                <div class="col-md-4" id="descriptionResponsable" style="display: none;">
                                    <label id="titleResponsable" class="form-label">Responsable del tratamiento</label>
                            ...
                            if (isset($aleatorio)) {
                                ...
                                <input type="hidden" name="descripcionTratamiento" value="<?php echo $descripcionTratamiento; ?>" />
                                ...
                                <div class="col-md-4">
                                    <label class="form-label">Firma del cliente</label>
                                    <div class="firma" style="position: relative; width: 300px; height: 150px; -moz-user-select: none; -webkit-user-select: none; -ms-user-select: none; user-select: none;">
                                        <canvas id="signature-pad2" class="signature-pad" style="position: absolute; left: 0; top: 0; width: 300px; height: 150px; background-color: #cccccc; border-color: black; border-width: 4px; border-style: solid">
                                        </canvas>
                                        <input type="hidden" name="firma_img_consen" id="firma_img_consen" value="" />
                                    </div>
                                    <span class="label label-sm label-danger">
                                        <a href="#" id="clear" onclick="clearSignature();return false;" style="color: red; font-weight: bold;">Borrar Firma</a>
                                    </span>
                                </div>
                            <?php } ?>
                        </div>
                        ...
                        <?php if ($this->session->userdata('errorform') != '') { ?>
                            <div class="col-12">
                                <div class="alert alert-dismissible alert-primary d-flex flex-column flex-sm-row p-5 mt-10">
                                    <div class="d-flex flex-column pe-0 pe-sm-10 justify-content-center text-uppercase">
                                        <?= $this->session->userdata('errorform') ?></div>
                                    <button type="button" class="position-absolute position-sm-relative m-2 m-sm-0 top-0 end-0 btn btn-icon ms-sm-auto" data-bs-dismiss="alert">
                                        <i class="fa-times fas fs-3 text-primary"></i>
                                    </button>
                                </div>
                    ...
                    <div class="row mb-5 border-bottom">
                        ...
                        <div class="col-12">
                            ...
                            <select class="form-select form-select-solid" data-control="select2" name="tipo" id="edit_tipo_doc">
                                <?php $tipos = $tiposDocumentosPopup;
                                foreach ($tipos as $key => $value) { ?>
                                    <option value="<?= $value ?>"><?= $value ?></option>
                                <?php } ?>
                            </select>
...
<div class="modal fade" id="modal-presupuesto" tabindex="-1" aria-labelledby="modal-presupuestoLabel" aria-hidden="true" data-focus-on="input:first">
    <div class="modal-dialog modal-xl">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title text-center text-uppercase w-100" id="exampleModalLabel">Detalle</h5>
                <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
            </div>
...
<div class="modal fade" id="modal-evolutivo" tabindex="-1" aria-labelledby="modal-evolutivoLabel" aria-hidden="true" data-focus-on="input:first">
    <div class="modal-dialog modal-xl">
        <div class="modal-content">
            <form id="form_evolutivo" action="<?php echo base_url(); ?>Clientes/nuevo_evolutivo" method="post" name="form_evolutivo">
                <div class="modal-header">
...
<script>
    $(document).on('click', '#nueva-nota-interna', function() {
        ...
        $('#form_nota_interna').attr('action', '<?php echo base_url(); ?>Clientes/nueva_nota_interna');
    ...
    $(document).on('click', '[editar_nota_interna]', function(){
        ...
        $('#form_nota_interna').attr('action', '<?php echo base_url(); ?>Clientes/editar_nota_interna');
    ...
    $(document).on('click', '[borrar_nota_interna]', function() {
        if (confirm("¿DESEA MARCAR COMO BORRADO EL REGISTRO?")) {
            var url = '<?= base_url() ?>Clientes/borrar_nota_interna/' + $(this).attr('borrar_nota_interna') + '/<?=$id_cliente?>';
    ...
    $(document).ready(function() {
        xtab = <?php if (isset($aleatorio)) echo $aleatorio;
                else echo 0; ?>;
        console.log('xtab ' + xtab);
        if (parseInt(xtab) > 0) {
            console.log('paso');
            $('.nav-stretch a[href="#tab10default"]').tab('show');
        }
    });
    /** 08/11/23 Tratamiento para cada consentimiento */
    $('#selectConsentimniento').change(function() {
        titleTratatmiento = document.getElementById('titleTratamiento');
        titleProc = document.getElementById('titleProc');
        titleRiesgos = document.getElementById('titleRiesgos');
        titleProcTera = document.getElementById('titleTera');
        titleProbc = document.getElementById('titleProbc');
        document.getElementById('descriptionTrata').style.display = "none";
        opcion = parseInt($(this).val());
        if (opcion === 9 || opcion === 11 || opcion === 21 || opcion === 25) {
            document.getElementById('descriptionTrata').style.display = "block";
            titleTratatmiento.innerHTML = "Dientes";
            $('input:text').attr("placeholder", "Indique los dientes");
        }

        if (opcion === 12) {
            document.getElementById('descriptionTrata').style.display = "block";
            titleTratatmiento.innerHTML = "Condiciones personales especiales";
            $('input:text').attr("placeholder", "Indique las condiciones especiales");

            document.getElementById('descriptionRiesgos').style.display = "block";
        }

        if (opcion === 18) {
            document.getElementById('descriptionTrata').style.display = "block";
            titleTratatmiento.innerHTML = "Condiciones personales especiales";
            $('#tratamiento').attr("placeholder", "Indique las condiciones especiales");

            document.getElementById('descriptionProc').style.display = "block";
            document.getElementById('descriptionProblema').style.display = "block";
            document.getElementById('descriptionProcTerapeuticos').style.display = "block";
            document.getElementById('descriptionRiesgos').style.display = "block";
        }

        if (opcion === 10 || opcion === 19 ||opcion === 20 || opcion === 22 || opcion === 33) {
            document.getElementById('descriptionTrata').style.display = "block";
            titleTratatmiento.innerHTML = "Condiciones personales especiales";
            $('#tratamiento').attr("placeholder", "Indique las condiciones especiales");
        ...
        if (opcion === 12 || opcion === 15) {
            document.getElementById('descriptionTrata').style.display = "block";
            titleTratatmiento.innerHTML = "Condiciones personales especiales";
            $('#tratamiento').attr("placeholder", "Indique las condiciones especiales");

            document.getElementById('descriptionProcTerapeuticos').style.display = "block";
            titleProcTera.innerHTML = "Posibles riesgos añadidos debidos al procedimiento";
            $('#procTerapeuticos').attr("placeholder", "Describa los posibles riesgos");
        ...
        if (opcion === 16) {
            ...
            document.getElementById('descriptionRiesgos').style.display = "block";
            document.getElementById('descriptionProblema').style.display = "block";
            titleProbc.innerHTML = "Riesgos específicos en base a estado del paciente";
            $('#problema').attr("placeholder", "Describa los posibles riesgos");
        }
        ...
        if (opcion === 14) {
            ...
            document.getElementById('descriptionProblema').style.display = "block";
            document.getElementById('descriptionProc').style.display = "block";
            titleProc.innerHTML = "Riesgos que puede comportar el tratamiento";
            $('#procedimiento').attr("placeholder", "Indique los riesgos");
        }

        if (opcion === 24 || opcion === 26) {
            document.getElementById('descriptionTrata').style.display = "block";
            titleTratatmiento.innerHTML = "Dientes";
            $('input:text').attr("placeholder", "Indique los dientes");

            document.getElementById('descriptionRiesgos').style.display = "block";
            titleRiesgos.innerHTML = "Condiciones personales especiales";
            $('#riesgos').attr("placeholder", "Indique las condiciones especiales");
        }
    });
    ...
    $(document).on('click', '.editevol', function() {
        ...
        <?php
        /* Ya no limitamos porque si aparece el boton es porque es el master o porque es un empleado que puede editar el mismo dia
        if ($this->session->userdata('id_perfil') != 0){
        ?>
            $('#form_evolutivo').find('[name="nota_escritor"]').prop('readonly',true).prop('disabled',true);
        <?php
        }
        */
        ?>
    ...
    $(document).on('click', '.nuevoevol', function() {
        ...
        $('#form_evolutivo').find('[name="nota_escritor"]').prop('readonly',false).prop('disabled',false);
        //$('#form_evolutivo').find('[name="fecha_nota"]').prop('disabled',true);
