<div class="card card-flush">
    <div class="card-body pt-6">
        <?php if ($accion == "nuevo") {
            $actionform = base_url() . 'aseguradoras/gestion/guardar';
        } else {
            $actionform = base_url() . 'aseguradoras/gestion/actualizar/' . $registros[0]['id_aseguradora'];
        } ?>
        <form id="form" action="<?php echo $actionform; ?>" role="form" method="post" name="form">
            <div class="row mb-5 border-bottom">
                <div class="col-md-6 mb-5">
                    <label class="form-label">Nombre aseguradora</label>
                    <input name="nombre" class="form-control form-control-solid" type="text"
                           value="<?= (isset($registros) && isset($registros[0])) ? $registros[0]['nombre_aseguradora'] : '' ?>" placeholder=""
                           required/>
                </div>
