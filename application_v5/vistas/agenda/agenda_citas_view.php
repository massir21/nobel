...
<div class="card card-flush m-5">
    <div class="card-body p-5">
        <form id="form_cita" action="<?php echo $formaction; ?>" role="form" method="post" name="form_cita"
            ...
            <?php if (isset($saldo_cliente)) { ?>
            <div class="row mb-5 align-items-end"> 
                <div class="col12 <?php if ($saldo_cliente <= 0) echo 'text-danger'; ?>">
                    <label><strong>Saldo actual:</strong></label> <?php echo euros($saldo_cliente); ?>
                </div>
            </div>
            <?php } ?>
            ...
            <?php if (!isset($presupuesto_items)) { ?>
                <?php /* 20240202 - Añadir tarifa */ ?>
                <div class="row mb-5 align-items-end" style="display: none">
                    <?php /*ocultado al usuario, pero se sigue enviado el valor de id_tarifa*/ ?>
                    <div class="col-8">
                        <label for=""
                               class="form-label">Tarifas: </label>
                        <select name="id_tarifa" id="id_tarifa" class="form-select form-select-solid w-auto"
                                onchange="RecargaTarifa(event);"
                        >
                            <option value="0">Base</option>
                            <?php if (isset($tarifas)) {
                                if ($tarifas != 0) {
                                    foreach ($tarifas as $key => $row) { ?>
                                        <option value="<?php echo $row['id_tarifa']; ?>" <?= (isset($cita[0]['id_tarifa']) && ($row['id_tarifa'] == $cita[0]['id_tarifa'])) ? "selected" : '' ?>>
                                            <?php echo strtoupper($row['nombre_tarifa']); ?>
                                        </option>
                                    <?php }
                                }
                            } ?>
                ...
                <?php /* 20240202 - Fin Añadir tarifa */ ?>
                <div class="row mb-5 align-items-end">
            ...
            if ($xcitas > 1) { ?>
            <?php
            /* CHAINS 20240127: Mostrar otras citas completas y permitir cambiar todo */
            $thiscitagroup=Agenda_model::getCitaGroupValue($cita[0]);
            ?>
            <div class="row mt-5 pt-5 border-top">
                <h4>Otras Citas Programadas:</h4>
                <span class="small">Seleccione para aplicar cambios a todas las citas</span>
                    ...
                    <?php foreach ($otrascitas as $key => $row) {
                        if ($row['id_cita'] != $citaactual) { ?>
                            <li class="list-group-item">
                                <?php
                                $otracitagroup=Agenda_model::getCitaGroupValue($row);

                                ?>
                                <input type="checkbox" name="otrascitasacambiar[<?php echo $row['id_cita'];?>]" value="1"
                                    <?php echo $otracitagroup==$thiscitagroup ? 'checked="checked" ' : '' ; ?> />

                                <?php echo $row['fecha_inicio']; ?> a
                                las <?php echo $row['hora_inicio']; ?> -> <?php echo $row['servicio'] ? $row['servicio'] : 'Sin Servicio asociado'; ?> <?php
                                echo isset($row['dientes']) ? ' (PIEZA Nº:' . $row['dientes'] . ')' : '' ; ?>

                                <?php
                                if($otracitagroup==$thiscitagroup){
                                    ?>
                                    <button class="btn btn-sm btn-dark text-inverse-success m-2" type="button" onclick="DesagruparCita(<?php echo $row['id_cita'];?>)">
                                        Desagrupar
                                    </button>
                                    <?php
                                    // CHAINS 20240129 - Quiar este boton
                                    /*
                                    ?>
                                    <button class="btn btn-sm btn-icon btn-info ms-3" data-bs-toggle="tooltip" data-bs-placement="top" title="Editar cita"
                                            onclick="EditarOtraCita(<?php echo $row['id_cita'];?>)"><i class="fa fa-pencil" aria-hidden="true"></i>
                                    </button>
                                    <?php
                                    */
                                    // FIN CHAINS 20240129
                ...
                <?php
                /* FIN CHAINS 20240127: Mostrar otras citas completas y permitir cambiar todo */
                ?>
                <?php }
                } ?>
                ...
                <div class="row">
                    <div class="col-md-12 text-center">
                        ...
                        <?php } else { ?>
                            ...
                            // CHAINS Se elimina el boton Finalizar
                            if (false && isset($presupuesto_seleccionado) && isset($servicio_presupuesto) && (($servicio_presupuesto['coste'] <= $presupuesto_seleccionado['total_pagado'] - $presupuesto_seleccionado['total_gastado']) || $presupuesto_seleccionado['es_repeticion'] == 1)) { ?>
...
<script>
    ...
    ?>
    function DesagruparCita(id){
        if (confirm("¿Desea realmente DESAGRUPAR la cita del grupo actual?")) {
            document.form_cita.action = "/agenda/citas/desagrupar/"+id;
            document.form_cita.submit();
        }
    }

    function EditarOtraCita(id){
        window.opener.CitasEditar(id);
        window.opener.location.reload();
        window.close();
    }
    <?php
    ...
    function Finalizar() {
        alert('Deshabilitado');
        <?php
        /* CHAINS Se deshabilita la opción
        Swal.fire({
