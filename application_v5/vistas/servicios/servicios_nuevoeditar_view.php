    ...
    <div class="card-body pt-6">
        ...
        <form id="form" action="<?php echo $actionform; ?>" role="form" method="post" name="form">
            <div class="row mb-5 border-bottom">
                ...
                <div class="col-md-3 mb-5">
                    <label class="form-label">MAX. DESCUENTO</label>
                    <input name="maxdescuento" class="form-control form-control-solid" type="number" step="0.01" min="0" max="100" value="<?= (isset($registros)) ? $registros[0]['maxdescuento'] : '' ?>" placeholder="" required />
                </div>

                <?php
                // CHAINS 20240201 - Quitar del formulario lo dejo como hidden porque es obligatorio
                /*
                ?>
                <div class="col-md-3 mb-5">
                ...
                */
                ?>
                <input type="hidden" name="precio_proveedor" value="0.0" />
                ...
                */
                ?>
                <input type="hidden" name="templos" value="" />
                <?php
                if(isset($tarifas)){
                    if(count($tarifas)){
                        foreach($tarifas as $tarifa){
                            ?>
                            <div class="col-md-3 mb-5">
                                <label class="form-label">P.V.P <i><?php echo $tarifa['abreviatura'];?></i></label>
                                <input name="pvptarifas[<?php echo $tarifa['id_tarifa'];?>]" class="form-control form-control-solid" type="number" step="0.01"
                                       value="<?php
                                       if(isset($precios) && is_array($precios)){
                                           foreach($precios as $precio){
                                               if($precio['id_tarifa']==$tarifa['id_tarifa']){
                                                   echo $precio['pvp'];
                                                   break;
                                               }
                                           }
