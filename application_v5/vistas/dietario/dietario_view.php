            ...
            <?php if ($this->session->userdata('id_perfil') == 0) { ?>
                <div class="w-auto ms-3">
                    ...
                    <select name="id_usuario" id="id_usuario" class="form-select form-select-solid w-auto">
                        ...
                        <?php if (isset($usuarios_no_borrados)) {
                            if ($usuarios_no_borrados != 0) {
                                foreach ($usuarios_no_borrados as $key => $row) {
                                    if ($row['id_usuario'] > 1) { ?>
                                        <option value='<?php echo $row['id_usuario']; ?>' <?php if (isset($id_usuario)) {
...
<script>
    ...
    */ ?>
    ...
    function NuevoDiaFiltroCentro() {
        document.location.href = "<?php echo base_url(); ?>dietario/index/dia/" + document.getElementById("fecha").value + "/" + document.getElementById("id_centro").value + "/" + document.getElementById("id_usuario").value;
    }
