...
//Alfonso Medina
// Control de pago
// Pagos con transferencia o financiado
// Subir comprobante
?>
<style>
    .dataTables_filter {
        ...
        <div class="card-toolbar flex-row-fluid justify-content-end gap-5 align-items-end">
            ...
            <div class="w-auto">
                ...
                <select name="tipo_filtro" id="tipo_filtro" class="form-select form-select-solid w-auto">
                    <option value="">Todos</option>
                    <option value="Ver justificante">Entregado</option>
                    <option value="PENDIENTE JUSTIFICANTE">Pendiente</option>
                </select>
            </div>
            <div class="w-auto">
                ...
                <input type="date" id="fecha" name="fecha" value="<?php if (isset($hoy_aaaammdd)) {
                                                                        echo $hoy_aaaammdd;
                                                                    } ?>" class="form-control form-control-solid w-auto" placeholder="desde" required />
            </div>
            <div class="w-auto">
                <label for="" class="form-label">Hasta</label>
                <input type="date" id="fecha2" name="fecha2" value="<?php if (isset($hoy_aaaammdd2)) {
                                                                        echo $hoy_aaaammdd2;
                                                                    } ?>" class="form-control form-control-solid w-auto" placeholder="Hata" required />
            </div>
            <div class="w-auto  ms-3">
                <?php if ($this->session->userdata('id_perfil') == 0) { ?>
            ...
            <table id="dietario" class="table align-middle table-striped table-row-dashed fs-6 gy-5">
                ...
                <tbody class="text-gray-700 fw-semibold">
                    <?php if (isset($dietario)) {
                        if ($dietario != 0) {
                            foreach ($dietario as $key => $row)if($row['pago_a_cuenta'] == 1&&$row['estado']=='Pendiente justificante'||$row['justificante_pagado'] != '') { 
                                ...
                                <tr style="background: <?php echo $row['color_estado']; ?>;">
                                    ...
                                    <td>
                                        <?php if ($row['tipo_pago'] == "#templos") { ?>
                                            <a class="text-dark fw-bold d-block mb-1 fs-6" href="<?php echo base_url(); ?>dietario/ticket_today/<?php echo $row['fecha_hora_concepto_aaaammdd'] . "/" . $row['id_cliente']; ?>" target="_blank"><?php echo $row['fecha_hora_concepto_ddmmaaaa_abrv']; ?> <?php echo $row['hora']; ?></a>
                                        <?php } else { ?>
                                            <span class="text-dark fw-bold d-block mb-1 fs-6"><?= $row['fecha_hora_concepto_ddmmaaaa_abrv'] ?> <?php echo $row['hora']; ?></span>
                                        <?php } ?>
                            ...
                            <!--                <button type="button" onclick="Generarticket(<?php echo $row['id_cliente']; ?>,<?php echo $row['id_centro']; ?>);" class="btn btn-icon btn-sm btn-primary" data-bs-toggle="tooltip" title="Generar ticket"><i class="fas fa-ticket"></i></button>-->
                                    ...
                                    <?php if ($row['estado'] != "Cierre Caja") { ?>
                                        ...
                                        <td class="text-end">
                                            <?php
                                            if($row['comisionfinanciacion']>0 ||
                                                ($row['comisionfinanciacion']==0 && !empty($row['justificante_pagado']))){
                                                ?>
                                                    <a href="javascript:void(0);"
                                                       data-commision="<?php echo $row['comisionfinanciacion'];?>"
                                                       class="link_presu_financiacion" data-iddietario="<?php echo $row['id_dietario'];?>"><?php
                                                echo
                                                    $row['comisionfinanciacion']>0 ?
                                                    round($row['comisionfinanciacion'],2)." €" :
                                                    "N/A";
                                                ?></a>
                                                <?php
                                            }
                                    ...
                                    <?php } else { ?>
                                        ...
                                        <td>-</td>
                                        <td>-</td>
                                        <td class="text-end"><?php echo $row['comisionfinanciacion'] ? round($row['comisionfinanciacion'],2)." €" : "";?></td>
                                        <td>-</td>
                                    <?php } ?>
...
<script>
    ...
    */ ?>
    ...
    function NuevoDia() {
        document.location.href = "<?php echo base_url(); ?>dietario/control_pagos/dia/" + document.getElementById("fecha").value;
    }

    function NuevoDiaFiltroCentro() {
      $(".loader_balance").fadeIn(200);
        document.location.href = "<?php echo base_url(); ?>dietario/control_pagos/dia/" + document.getElementById("fecha").value + "/" + document.getElementById("fecha2").value + "/"+ document.getElementById("id_centro").value;
    }
    ...
    */
    ...
    jQuery(document).ready(function(){
       jQuery(".link_presu_financiacion").on('click',function(){
           var id_dietario=jQuery(this).data('iddietario');
           var comm=jQuery(this).data('commision');
           var xurl = "<?php echo base_url(); ?>presupuestos/json_getfinanciacion/" + id_dietario ;
           $.ajax({
               data: {},
               url: xurl,
               type: 'get',
               ...
               success:    function (data) {
                   ...
                   if(typeof data.error != "undefined"){
                       ...
                       if(data.result==0 || data.result.length==0){
                           ...
                           var commfield=``;
                           if(comm<=0){
                               commfield=`<div class="mb-3">
                               <label for="comisionfinanciacion" class="form-label">Gastos de transacción</label>
                               <input type="number" class="form-control" id="comisionfinanciacion" name="comisionfinanciacion2" required>
                                   <span class="text-muted" id="comisionfinanciacion"></span>
                                </div>`;
                           }

                           const modalContent = `
                                 <div class="modal modal-lg fade" id="modalFormularioFinanciacion" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true" style="z-index:99999;">
                                     <div class="modal-dialog">
                                        <div class="modal-content">
                                            <div class="modal-header">
                                                <h5 class="modal-title" id="exampleModalLabel">Los gastos de financiación no están asociados a ningun presupuesto</h5>
                                                <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                                            </div>
                                            <div class="modal-body">
                                                <p>Seleccione un presupuesto al que asociar los gastos</p>
                                                <form id="justificante-financiacion">`+
                                                    commfield+
                                                    `<div class="mb-3" >
                                                    ...
                                                    <button type="button" class="btn btn-primary" id="enviar_financiacion">Enviar</button>
                                                </form>
                                            </div>
                                        </div>
                                    </div>
                           ...
                           $("#modalFormularioFinanciacion").on('shown.bs.modal', function () {
                           ...
                           $("#modalFormularioFinanciacion").modal('show');
                           $('.modal-backdrop.fade.show').attr('style', 'z-index:9999;')
                           $("#enviar_financiacion").click(function(event) {
                               var formData = new FormData(document.getElementById('justificante-financiacion'));
                               formData.append("id_dietario", id_dietario);
                               formData.append("id_presupuesto", id_presupuesto);
                               $.ajax({
                                   headers: {
                                   ...
                                   method: 'post',
                                   url: '<?php echo base_url() ?>Presupuestos/asignarFinanciacionPresu',
                                   data: formData,
                                   processData: false,
                                   contentType: false,
                                   success: function(resp) {
                                       ...
                                       if (resp == true) {
                                           Swal.fire({
                                               ...
                                               type: 'success',
                                               willClose: function() {
                                                   $("#modalFormularioFinanciacion").modal('hide');
                                                   window.location.reload();
                                               },
                                           });
                                       } else {
                           ...
                           $("#modalFormularioFinanciacion").on("hidden.bs.modal", function() {
                               $('.modal-backdrop.fade.show').removeAttr('style')
                               $(this).remove();
                           });
                           });
                       ...
                       var htmltable=`<div class="table-responsive">
                           ...
                           <th scope="col">#NUM. PRES.</th>
                            <th scope="col">ESTADO</th>
                       <th scope="col">CREACION</th>
                       <th scope="col">VALIDEZ</th>
                       <th scope="col">TOTAL</th>
                        <th scope="col">COMISION FINANCIACION</th>
                   </tr>
                   </thead>
                       <tbody>
                    `;
                     for(var i=0;i<data.result.length;i++){
                         ...
                         var htmlcliente=row.nombre+` `+row.apellidos
                         var htmlPresu= row.nro_presupuesto;
                         /* if (row.estado == 'Pendiente') {
                             htmlPresu += ' <span class="badge badge-warning ms-2" data-bs-toggle="tooltip" title="' + row.estado + '"><i class="fa fa-clock text-white"></i></span>';
                         } else if ((row.estado == 'Aceptado' || row.estado == 'Aceptado parcial')&&row.items_sinfinalizar > 0) {
                             htmlPresu += ' <span class="badge badge-success ms-2" data-bs-toggle="tooltip" title="' + row.estado + '"><i class="fa fa-check text-white"></i></span>';
                         }  else if ((row.estado == 'Aceptado' || row.estado == 'Aceptado parcial')&&row.items_sinfinalizar == 0) {
                             htmlPresu += ' <span class="badge badge-primary ms-2" data-bs-toggle="tooltip" title="Finalizado"><i class="fa fa-check text-white"></i></span>';
                         } else if (row.estado == 'Rechazado') {
                             htmlPresu += ' <span class="badge badge-danger ms-2" data-bs-toggle="tooltip" title="' + row.estado + '"><i class="fa fa-times text-white"></i></span>';
                         } else {
                             htmlPresu += ' <span class="badge badge-secondary ms-2" data-bs-toggle="tooltip" title="' + row.estado + '"><i class="fa fa-clock text-dark"></i></span>';
                         } */

                         htmltable+=`<tr>`;
                         htmltable+=`<th class="text-center" scope="row">`+htmlPresu+`</th>`;
                         htmltable+=`<th class="text-center" scope="row">`+row.estado+`</th>`;
                         /* htmltable+=`<td>`+row.nombre+` `+row.apellidos+`</td>`; */
                         htmltable+=`<td>`+row.fecha_creacion+`</td>`;
                         htmltable+=`<td>`+row.fecha_validez+`</td>`;
                         htmltable+=`<td >`+row.totalpresupuesto+` €</td>`;
                         htmltable+=`<td >`+row.totalcomisionfinanciacion+` €</td>`;
                         htmltable+=`</tr>`;
                     }
                   ...
                   htmltable+=`
                       ...
                       const modalContent = `
                <div class="modal modal-lg fade" id="modalInfoFinanciacion" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true" style="z-index:99999;">
                    <div class="modal-dialog">
                        <div class="modal-content">
                            <div class="modal-header">
                                <h5 class="modal-title" id="exampleModalLabel">Presupuestos financiados con la entrada de dietario de `+htmlcliente+`</h5>
                                <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                            </div>
                            <div class="modal-body">
                                `+htmltable+`
            ...
            `;
                       ...
                       $("#modalInfoFinanciacion").modal('show');
                       $('.modal-backdrop.fade.show').attr('style', 'z-index:9999;')
                   }
               }
           });
