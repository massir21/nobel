...
<script>
        ...
        columns: [{
            ...
            },
            {
                ...
                render: function(data, type, row) {
                    ...
                    if(row.otrospresus.length>0){
                        html+='<br/>';
                        var cborrador=cpendiente=caceptado=crechazado=cotros=0;
                        for(var i=0;i<row.otrospresus.length;i++){
                            if(row.otrospresus[i].estado=='Borrador') cborrador++;
                            else if(row.otrospresus[i].estado=='Pendiente') cpendiente++;
                            else if (row.otrospresus[i].estado=='Aceptado' || row.otrospresus[i].estado=='Aceptado parcial') caceptado++;
                            else if(row.otrospresus[i].estado=='Rechazado') crechazado++;
                            else cotros++;
                        }
                        if(cborrador>0) {
                            html += '<a class="btnotropresu" target="_blank" href="<?= base_url() ?>Presupuestos/?id_cliente='+row.id_cliente+'"><span class="badge badge-circle badge-outline badge-dark" title="'+cborrador+' presupuestos más en borrador">' +cborrador + '</span></a>'
                        }
                        if(cpendiente>0){
                            html += '<a class="btnotropresu" target="_blank" href="<?= base_url() ?>Presupuestos/?id_cliente='+row.id_cliente+'"><span class="badge badge-circle badge-outline badge-warning" title="'+cpendiente+' presupuestos más pendeintes">' + cpendiente + '</span></a>'
                        }
                        if(caceptado>0){
                            html += '<a class="btnotropresu" target="_blank" href="<?= base_url() ?>Presupuestos/?id_cliente='+row.id_cliente+'"><span class="badge badge-circle badge-outline badge-success" title="'+caceptado+' presupuestos más aceptados">' + caceptado + '</span></a>'
                        }
                        if(crechazado>0){
                            html += '<a class="btnotropresu" target="_blank" href="<?= base_url() ?>Presupuestos/?id_cliente='+row.id_cliente+'"><span class="badge badge-circle badge-outline badge-danger" title="'+crechazado+' presupuestos más rechazados">' + crechazado + '</span></a>'
                        }
                        if(cotros>0){
                            html += '<a class="btnotropresu" target="_blank" href="<?= base_url() ?>Presupuestos/?id_cliente='+row.id_cliente+'"><span class="badge badge-circle badge-outline badge-light" title="'+cotros+' presupuestos más enotros estados">' + cotros + '</span></a>'
                        }
   ...
   /* $(document).on('click', '.btnotropresu', function(event) {
        var button = $(this);
        var idcleinte = jQuery(this).data('idcliente');
        var url = '<?= base_url() ?>Presupuestos/?id_cliente=' + idcleinte;
        //window.location.href = url;
        var posicion_x;
        var posicion_y;
        var ancho = 800;
