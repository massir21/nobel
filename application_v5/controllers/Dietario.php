...
*/
    ...
    public function get_presupuestos($table = null, $columna = null, $valor = null)
    {
                ...
                if(!in_array($unresult->id_cliente,$buscarPresupuestoPara)) {
                    ...
                    if($presupuestos > 0 ){
                        ...
                        $unresult->descuento = round($presupuestos[0]['dto_100'] > 0 ? $presupuestos[0]['dto_100'] :
                            ($presupuestos[0]['total_sin_descuento'] !=0 ? ($presupuestos[0]['total_sin_descuento'] - $presupuestos[0]['totalpresupuesto']) / $presupuestos[0]['total_sin_descuento'] * 100 : 0),2);
       ...
       /*if($_SERVER['REMOTE_ADDR']=='212.231.230.131'){
...
// Alfonso: Duetario de control de pagos de transferencia o financiado (ue requieren comprobante)
    ...
    function pago_ficticio()
    {
        $this->load->model('Dietario_model');
        $this->load->model('Clientes_model'); 
        $id_cliente = $this->input->post('id_cliente');
        $pagado_tarjeta = $this->input->post('pagado_tarjeta');
        $pagado_efectivo = $this->input->post('pagado_efectivo');
        $pagado_tpv2 = $this->input->post('pagado_tpv2');
        $pagado_paypal = $this->input->post('pagado_paypal');
        $pagado_transferencia = $this->input->post('pagado_transferencia');
        if(empty($pagado_tarjeta)) { $pagado_tarjeta=0; }
        if(empty($pagado_efectivo)) { $pagado_efectivo=0; }
        if(empty($pagado_tpv2)) { $pagado_tpv2=0; }
        if(empty($pagado_paypal)) { $pagado_paypal=0; }
        if(empty($pagado_transferencia)) { $pagado_transferencia=0; }
        $importe=$pagado_tarjeta+$pagado_efectivo+$pagado_tpv2+$pagado_paypal+$pagado_transferencia;
        $data['id_cliente'] = $id_cliente;
        $data['pagado_tarjeta'] = $pagado_tarjeta;
        $data['pagado_efectivo'] = $pagado_efectivo;
        $data['pagado_tpv2'] = $pagado_tpv2;
        $data['pagado_paypal'] = $pagado_paypal;
        $data['pagado_transferencia'] = $pagado_transferencia;
        $data['id_pedido']=0;
        $data['id_ticket']=0;
        $data['id_cita']=0;
        $data['recarga_templos']=0;
        $data['id_presupuesto']=0;
        $data['id_presupuesto_pago']=0;
        $data['pago_a_cuenta']=0;
        $data['descuento_euros']=0;
        $data['descuento_porcentaje']=0;
        $data['pagado_habitacion']=0;
        $data['pagado_financiado']=0;
        ...
        $data['facturado']=0;
        $data['desdebug']=0;
        $id_dietario=$this->Dietario_model->crearDietarioFicticio($data);
        $data2['id_cliente'] = $id_cliente;
        $data2['id_dietario'] = $id_dietario;
        $data2['importe']=$importe;
        $data2['tipo_pago']='#tarjeta';
        $data2['motivo']='Pago ficticio';
        $data2['fecha_creacion']='0000-00-00 00:00:00';
        $data2['id_usuario_creacion']=0;
        $data2['debug']=0;
        $saldo_ficticio=$this->Clientes_model->crearSaldoFicticio($data2);
        $response = array('success' => true, 'error' => false,'data' => ['id_dietario' => $id_dietario, 'saldo_ficticio' =>$saldo_ficticio]);
