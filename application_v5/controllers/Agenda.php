...
class Agenda extends CI_Controller
{
    ...
    function index($fecha = null, $id_centro = null)
    {
        ...
        $parametros['obtenerAgrupado']=true;
        // FIN CHAINS 20240127: Se añade el parametro para obtener los eventos agrupados
            ...
            foreach ($data['citas_agenda'] as $key => $cita) {
                if ($cita['id_presupuesto'] != '') {
                    ...
                    if (array_key_exists($cita['id_presupuesto'], $presupuestos_dia)) {
                        ...
                        $xpresupuesto=$this->Presupuestos_model->leer_presupuestos($param5);
                        if($xpresupuesto>0){
                            $presupuesto=$xpresupuesto[0];
        ...
        if ($data['empleados'] != 0) {
            foreach ($data['empleados'] as $key => $row) {
		        ...
		        $xparams=[];
        if ($this->session->userdata('id_perfil')==6){
            $xparams['id_doctor']=$this->session->userdata('id_usuario');
        }
        else
            if ($this->session->userdata('id_perfil')==1 || $this->session->userdata('id_perfil')==2 || $this->session->userdata('id_perfil')==3){
                $xparams['id_centro']=$this->session->userdata('id_centro_usuario');
            }
            else
        ...
        $data['citaspendientesnota']=$this->Agenda_model->get_citas_sin_nota_por_fecha($xparams);
    ...
    function citas($accion = null, $id_cita = null, $id_empleado = null, $fecha_cita = null, $hora_cita = null, $id_cliente_cita = null)
    {
        ...
        // ... Nueva cita
        // ---------------------------------------
         // CHAINS 20240128 - Desagrupar cita
        if($accion=="desagrupar"){
            unset($parametros);
            unset($param);
            $param['id_cita'] = $id_cita;
            $micita = $this->Agenda_model->leer_citas($param);
            if(is_array($micita) && count($micita)) {
                $micita=$micita[0];
                unset($micita['groupcodemd5']);
                $micita['groupcodemd5'] = Agenda_model::getCitaGroupValue($micita, true);
                $micita['id_empleado']=$micita['id_usuario_empleado'];
                $micita['fecha']=date("Y/m/d",strtotime($micita['fecha_inicio']));
                $micita['hora']=date("H",strtotime($micita['fecha_hora_inicio']));
                $micita['recordatorio_email'] = $micita['recordatorio_email']==0 ? '' :$micita['recordatorio_email'];
                $micita['recordatorio_sms'] = $micita['recordatorio_sms']==0 ? '' :$micita['recordatorio_sms'];
                $micita['duracion_nueva']=$micita['duracion'];
                $this->Agenda_model->modificar_cita($micita);
            }
        }
        // FIN CHAINS 20240128 - Desagrupar
        else
		if ($accion == "nuevo") {
            unset($parametros);
            $parametros = $_POST;
            ...
            if (isset($parametros)) {
                ...
                if (isset($parametros['id_cliente'])) {
                    ...
                    $data['saldo_cliente'] = $this->Clientes_model->saldo($parametros['id_cliente']);
        ...
        if ($accion == "editar") {
            ...
            // CHAINS 20240129 - Fecha desde el dia anterior de la cita
			$param10['fecha_desde'] = date('Y-m-d',strtotime('-1 days',strtotime($data['cita'][0]['fecha_hora_inicio'])));
        ...
        if ($accion == "guardar") {
            ...
            $parametros = $_POST;
            //printr($parametros);
			// CHAINS 20240127 -- Guardamos los IDs de las citas creadas pra excluirlas de la busqueda
            $idSCitasCreadas=[];
            // FIN CHAINS 20240127 --
                ...
                if (isset($parametros['id_presupuesto_item']) && count($parametros['id_presupuesto_item']) > 0) {
                    ...
                    foreach ($parametros['id_presupuesto_item'] as $key => $presupuesto_item) {
                        ...
                        if (isset($parametros['recordatorio_email'])) {
						 ...
						 // CHAINS 20240127 - Añadimos los ids de las citascreadas para que no la tenga en cuenta al buscar si ya existe
                        if(count($idSCitasCreadas)) $param['id_cita_excluir']=$idSCitasCreadas;
                        // FIN CHAINS 20240127
                        ...
                        if(key_exists('id_tarifa',$parametros)){
                            ...
                            $this->load->model('Presupuestos_model');
                            $presupuesto=$this->Presupuestos_model->leer_presupuestos(['id_presupuesto'=>$id_presupuesto]);
                            if($presupuesto){
                                if(isset($presupuesto[0])){
                                    if(isset($presupuesto[0]['id_tarifa'])){
                                        $param['id_tarifa']=$presupuesto[0]['id_tarifa'];
                                    }
                                }
                            }
                        }
                        ...
                        if ($id_cita_creada > 0) {
                            ...
                            $idSCitasCreadas[]=$id_cita_creada;
                            // FIN CHAINS 20240127 -
                            $this->load->model('Presupuestos_model');
                        ...
                        $segundos_minutoAnadir = ($serv[0]['duracion']) * 60;
                         // CHAINS 20240127 -- Ya no añadimos nada a la hora para que se cren todas en la misma hora.
                        // $nuevahorainicio = date("H:i", $segundos_horaInicial + $segundos_minutoAnadir);
                         $nuevahorainicio = date("H:i", $segundos_horaInicial);
                        // FIN CHAINS 20240127 -- Ya no añadimos nada a la hora para que se cren todas en la misma hora.
                    }
                   ...
                   // CHAINS 20240128 - Añadimos los ids de las citascreadas para que no la tenga en cuenta al buscar si ya existe
                    if(count($idSCitasCreadas)) $param['id_cita_excluir']=$idSCitasCreadas;
                    // FIN CHAINS 20240128
                    ...
                    $segundos_horaInicial = strtotime($param['hora']);
                    $segundos_minutoAnadir = ($serv[0]['duracion']) * 60;
                    // CHAINS 20240128 -- Ya no añadimos nada a la hora para que se cren todas en la misma hora.
                    // $param['hora'] = date("H:i", $segundos_horaInicial + $segundos_minutoAnadir);
                    $param['hora'] = date("H:i", $segundos_horaInicial);
                    // FIN CHAINS 20240128 -- Ya no añadimos nada a la hora para que se cren todas en la misma hora.
                    // ---------------------------------------
        ...
        if ($accion == "modificar") {
            ...
            if (isset($parametros)) {
                ...
                // CHAINS 20240128 - Buscamos otras citas agrupadas
                $idSOtrasCitasACambiar=[];
                if(isset($parametros['otrascitasacambiar'])){
                    foreach($parametros['otrascitasacambiar'] as $idotracita => $cambiar) $idSOtrasCitasACambiar[]=$idotracita;
                }
                foreach($idSOtrasCitasACambiar as $idOtraCitaACambiar){
                    unset($param);
                    $param['id_cita']=$idOtraCitaACambiar;
                    $cita=$this->Agenda_model->leer_citas($param);
                    if(is_array($cita)){
                        $otrosparametros=[];
                        $otrosparametros['id_cita']=$idOtraCitaACambiar;
                        $otrosparametros['id_servicio']=$cita[0]['id_servicio'];
                        $otrosparametros['id_empleado']=$cita[0]['id_usuario_empleado'];
                        $otrosparametros['id_cliente']=$cita[0]['id_cliente'];
                        $otrosparametros['fecha']=$parametros['fecha'];
                        $otrosparametros['hora']=$parametros['hora'];
                        $otrosparametros['duracion_nueva']=$parametros['duracion_nueva'];
                        $ok = $this->Agenda_model->modificar_cita($otrosparametros);



                        unset($paramcitas);
                        $paramcitas['id_cita'] = $idOtraCitaACambiar;
                        $paramcitas['id_cliente'] = $cita[0]['id_cliente'];
                        $citas = $this->Agenda_model->leer_citas_correo($paramcitas);
                ...
                // FIN CHAINS 20240128 - Fin buscar otras citas agrupadas
             }
        }
        // ---------------------------------------
        ...
        if ($accion == "anular") {
            ...
            if (isset($parametros)) {
                ...
                // CHAINS 20240128 - Marcar como no vino todos los checkeadod
                $idsToProccess=[$id_cita];
                if(isset($_POST['otrascitasacambiar'])){
                    foreach($_POST['otrascitasacambiar'] as $otroid=>$val){
                        if($val) $idsToProccess[]=$otroid;
                    }
                }

                foreach($idsToProccess as $idc) {
                    unset($param);
                    $param['id_cita'] = $idc;
        ...
        if ($accion == "finalizar") {
            ...
            if (isset($parametros)) {
                ...
                /* CHAINS 2023 - Finalizar citas agrupadas */
                $idsToProccess=[$id_cita];
                if(isset($_POST['otrascitasacambiar'])){
                    foreach($_POST['otrascitasacambiar'] as $otroid=>$val){
                        if($val) $idsToProccess[]=$otroid;
                ...
                // Primero comprobamso que el saldo total sea suficiente
                $presupuestosLLL=[];
                $costesLLL=[];
                foreach($idsToProccess as $idc) {
                    unset($param2);
                    ...
                    if(!empty($enpresupuesto)){
                        ...
                        if(!isset($presupuestosLLL[$enpresupuesto[0]['id_presupuesto']])){
                            $presupuestosLLL[$enpresupuesto[0]['id_presupuesto']]=$pre[0];
                        }
                        if(!isset($costesLLL[$enpresupuesto[0]['id_presupuesto']]))  $costesLLL[$enpresupuesto[0]['id_presupuesto']]=0;
                        $costesLLL[$enpresupuesto[0]['id_presupuesto']]+=$enpresupuesto[0]['coste'];
                    }
                }

                foreach($presupuestosLLL as $id_pres => $pres){
                    $saldo_disponible = ($pres['es_repeticion'] == 1) ? $pres['coste'] : $pres['total_pagado'] - $pres['total_gastado'];
                    if($costesLLL[$id_pres]>$saldo_disponible){
                        $this->load->view('agenda/agenda_mensaje_view', ['mensaje' => 'No hay saldo suficiente en alguno de los presupuestos para finalizar las citas seleccionadas']);
                        return;
                    }
                }
                // Fin  comprobamso que el saldo total sea suficiente
                    ...
                    if (!empty($enpresupuesto)) {
                        ...
                        if ($saldo_disponible >= $enpresupuesto[0]['coste']) {
                            ...
                            */
                            ...
                            $this->Liquidaciones_model->liquidacion_cita($idc);
        ...
        // 20240202 CHAINS Tarifas
        $data['tarifas'] = $this->Tarifas_model->leer_tarifas(["activo" => 1]);
        // ---------------------------------------
        // ---------------------------------------
    ...
    function modificar_duracion()
    {
        ...
        if ($permiso) {
            ...
            // CHAINS 20240128 - Modificar duracion de todas las citas agrupadas
            $idsCitasACambiar=[];
            if(is_array($cita) && isset($cita[0])){
                $idsCitasACambiar[]=$parametros['id_cita'];
                unset($param);
                $param['groupcodemd5']=$cita[0]['groupcodemd5'];
                $param['fecha']=date("d/m/Y",strtotime($cita[0]['fecha_hora_inicio']));
                $param['hora']=date("H:i",strtotime($cita[0]['fecha_hora_inicio']));
                $param['programadas_finalizadas']=1;
                $param['id_cita_excluir']=$parametros['id_cita'];
                $otrascitas=$this->Agenda_model->leer_citas($param);
                if(is_array($otrascitas)){
                    foreach($otrascitas as $otracita){
                        $idsCitasACambiar[]=$otracita['id_cita'];
                    }
                }
            }


            foreach($idsCitasACambiar as $idCita){
                unset($param);
                $param['id_cita'] = $idCita;
