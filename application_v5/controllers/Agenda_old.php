...
class Agenda extends CI_Controller
{
	...
	function citas($accion = null, $id_cita = null, $id_empleado = null, $fecha_cita = null, $hora_cita = null, $id_cliente_cita = null)
	{
		...
		if ($accion == "editar") {
			...
			$data['codigo_proveedor'] = $cita_dietario[0]['codigo_proveedor'];
		...
		if ($accion == "finalizar") {
			...
			if (isset($parametros)) {
				...
				if(!empty($enpresupuesto)){
					...
					$saldo_disponible = $pre[0]['total_pagado'] - $pre[0]['total_gastado'];
	...
	//20/7/21 Para Doctor
	public function prueba_doctor($fecha = null, $id_centro = null)
	{
		$ok_ticket = $this->Ticket_model->recoger_ticket($this->session->userdata('ticket'));
		if ($ok_ticket == 0) {
			header("Location: " . RUTA_WWW);
		...
		$data['content_view'] = $this->load->view('agenda/doctor_view', $data, true);

		// ... Modulos del cliente
		$param_modulos['id_perfil'] = $this->session->userdata('id_perfil');
		//echo "Id perfil ".$this->session->userdata('id_perfil');
		$data['modulos'] = $this->Usuarios_model->leer_modulos($param_modulos);
		//echo "<pre>";
		...
		$permiso = $this->Acceso_model->TienePermiso($data['modulos'], 59);
		if ($permiso) {
			$this->load->view($this->config->item('template_dir') . '/master_agenda', $data);
		} else {
			header("Location: " . RUTA_WWW . "/errores/error_404.html");
